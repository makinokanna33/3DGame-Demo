...
public class MoveRangConStantPath : Path
{
    ...
    public override bool CanTraverse(GraphNode node)
    {
        ...
        // 假如人物会不会游泳就不能穿过带水的节点
        if (canSwim == false && node.Tag == (uint)GameDefine.AstartTag.Water)
        {
            return false;
        }
    ...
    public static MoveRangConStantPath Construct(Vector3 start, int maxGScore,bool canSwim, OnPathDelegate callback = null)
    {
        MoveRangConStantPath p = PathPool<MoveRangConStantPath>.GetPath();
        p.canSwim = canSwim;
