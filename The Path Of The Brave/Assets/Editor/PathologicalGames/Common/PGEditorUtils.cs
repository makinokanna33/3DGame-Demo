...
public static class PGEditorUtils 
{
	...
	/// <summary>
	/// A toggle toggle for a Bool type SerializedProperty. Nothing is returned because the 
                                          ...
                                          where T : new()    
    {
        ...
        if (!expanded)
        {
            // Don't add the '+' toggle when the contents are collapsed. Just quit.
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.EndHorizontal();
        ...
        // A little space between toggle groups
    ...
    private static bool AddFoldOutListHeader<T>(string label, List<T> list, bool expanded, int lastIndent)
    {
        ...
        EditorGUILayout.BeginHorizontal(GUILayout.MaxWidth(50));   // 1/2 the item toggle width
        GUILayout.Space(buttonSpacer);

        // Master add at end toggle. List items will insert
        if (GUILayout.Button(new GUIContent("+", "Click to add"), EditorStyles.toolbarButton))
            list.Add(default(T));
    ...
    /// <summary>
    /// Used by AddFoldOutListItemButtons to return which toggle was pressed, and by 
    /// UpdateFoldOutListOnButtonPressed to process the pressed toggle for regular lists
    /// </summary>
    ...
    private static LIST_BUTTONS AddFoldOutListItemButtons()
    {
        ...
        // Add toggle - Process presses later
        ...
        // Return the pressed toggle if any
    ...
    /// <summary>
    /// Used by basic foldout lists to process any list item toggle presses which will alter
